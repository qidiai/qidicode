...
pub const ENV_AUTO_GC: &str = "QIDI_WORKTREE_AUTO_GC";
/// `1` / `true` / `on` forces age-count without delete.
pub const ENV_AUTO_GC_DRY_RUN: &str = "QIDI_WORKTREE_AUTO_GC_DRY_RUN";
/// Default max age in seconds (overrides TOML/remote when set and parseable).
pub const ENV_AUTO_GC_MAX_AGE: &str = "QIDI_WORKTREE_AUTO_GC_MAX_AGE";
/// `1` / `true` / `on` enables optional discovery rebuild + stale git prune.
pub const ENV_AUTO_GC_REBUILD: &str = "QIDI_WORKTREE_AUTO_GC_REBUILD";

/// Remove every `QIDI_WORKTREE_AUTO_GC*` env var so a test starts from a clean
...
/// Parse `QIDI_WORKTREE_AUTO_GC_MAX_AGE` as seconds; invalid/absent → None.
...
mod tests {
    ...
    use cf_test_utils::git::{init_git_repo, run_git};
    ...
    /// Kill switch: env `QIDI_WORKTREE_AUTO_GC=0` or `opts.enabled=false` both
