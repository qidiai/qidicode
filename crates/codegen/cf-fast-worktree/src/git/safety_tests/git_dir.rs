...
fn lfs_object_beside_the_store_keeps_the_worktree() {
    ...
    for (name, worktree, surviving, storage) in [
        ...
        assert_eq!(
            ...
            Safety::Keep(KeepReason::WorktreeLocalState(
                // Reported with mixed separators on Windows: the `lfs/objects`
                // literal from git's own layout, then host-native `join`s.
                format!(
                    "lfs/objects/26{sep}cc{sep}26ccfbb9deadbeef",
...
fn submodule_store_the_source_does_not_hold_keeps_the_snapshot() {
    ...
    // Mixed separators on Windows (literal prefixes + host-native joins):
    // normalize before comparing.
    let held_norm = held.replace('\\', "/");
    assert!(
        held_norm.starts_with("modules/vendor/example-lib/"),
        "{held}"
    );
    assert!(worktree.exists());
}
