...
fn dirty_submodule_keeps_the_worktree_whatever_was_captured() {
    ...
    // Forward slashes: git config treats backslashes as escapes, so a Windows
    // path in `url = C:\Users\...` is a parse error ("bad config line").
    let upstream_url = upstream.display().to_string().replace('\\', "/");
    std::fs::write(
