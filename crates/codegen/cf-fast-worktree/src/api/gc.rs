...
pub struct GcReport {
    ...
    /// Grove pin-ref union-liveness sweep (`refs/qidi/worktrees/*`).
    #[serde(default)]
...
fn rec_cwd_within(rec: &crate::db::WorktreeRecord, live_cwds: &[PathBuf]) -> bool {
    ...
    if crate::nfs::is_grove_strategy(&rec.creation_mode) || dest_must_not_stat(path) {
    ...
    if opts.dry_run {
        ...
        let dead = all
            ...
            .filter(|rec| {
                ...
                if crate::nfs::is_grove_strategy(&rec.creation_mode)
...
// Upstream gc/tests.rs (integration suite: age-pass budgets, per-kind TTLs,
// gate interplay) depends on the upstream api.rs test fixtures
// (cwd_test_guard/CwdGuard) and db GrokHomeFixture::isolate_xdg_grove_data,
// which arrive with the discovery/rebuild pass in a later sync batch.
