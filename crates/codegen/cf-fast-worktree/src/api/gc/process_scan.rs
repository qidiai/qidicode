...
mod tests {
    ...
    #[cfg(windows)]
    #[test]
    fn is_pid_alive_windows_semantics() {
        assert!(is_pid_alive(std::process::id()), "our own pid is alive");
        assert!(!is_pid_alive(0), "pid 0 is the idle process, not tracked");
        // u32::MAX is way beyond the pid range; OpenProcess reports
        // ERROR_INVALID_PARAMETER for it, which reads as dead.
        assert!(!is_pid_alive(u32::MAX));
    }
