...
mod shell_suggestion_key_tests {
    ...
    // Terminal-like Tab is compiled out on Windows (cfg!(not(windows)) at the
    // dispatch site): the completion tokenizer/quoting is POSIX-only. Gate
    // these tests to POSIX so the Windows suite exercises the remaining flow.
    #[cfg(not(windows))]
    #[test]
